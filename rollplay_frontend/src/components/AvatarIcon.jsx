...
/**
 * Minimal avatar renderer for small tiles in the Arena.
 * Accepts a JSON string (from profiles.avatar_json) and renders a compact SVG.
 */

const DEFAULT_AVATAR_MODEL = {
  displayName: "Player",
  badge: "common",
  bodyShape: "round",
  skin: "#F2C7A5",
...
function layerHair(model) {
  ...
  if (style === "short") {
    path =
      "M115 165 C125 110 165 90 200 92 C250 95 285 125 290 165 C280 145 265 130 240 122 C220 115 185 118 165 128 C145 138 130 150 115 165 Z";
  } else if (style === "puff") {
    return `
    ...
    path =
      "M122 178 C120 130 155 92 205 92 C252 92 285 125 288 175 C290 220 276 270 268 298 C246 318 218 328 184 326 C154 322 132 308 118 288 C112 250 120 205 122 178 Z";
  }
  return `
    <g aria-label="Hair">
...
export default function AvatarIcon({ avatarJson, fallbackName }) {
  const model = useMemo(() => {
    if (!avatarJson) return DEFAULT_AVATAR_MODEL;
    try {
      return {
        ...DEFAULT_AVATAR_MODEL,
        ...(typeof avatarJson === "string" ? JSON.parse(avatarJson) : avatarJson),
      };
    } catch {
  ...
  // If we ever wanted pure initials fallback, we could check here; for now model always exists.
  return (
    <div
      style={{
        width: "100%",
        ...
        borderRadius: 24,
        overflow: "hidden",
        background: "radial-gradient(circle at 10% 0%, #20254a, #050815)",
        position: "relative",
      }}
    >
      <div
      ...
      />
      {/* scale down the SVG nicely */}
      <style>
        {`
        /* Limit any avatar SVG inside this icon */
        div[style*="radial-gradient"][style*="#20254a"] svg {
          width: 160%;
          height: auto;
